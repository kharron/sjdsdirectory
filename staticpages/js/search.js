	 ...
	 function create_list(res, type){
		 res_keys = Object.keys(res);
		 var html = '';
		 if (type == "products"){
			 res_keys.forEach(function(i){
				 html += "<li class='search-result'><a href='/search-result.html?product=" + res[i].id + "'>" + res[i].name + "</a></li>";
			 });
		 } else {
			 res_keys.forEach(function(i){
	 ...
	 function keyword_results(res, prods){
		 ...
		 html += create_list(res, 'business');
		 html += "<li style='height: 26px; background-color:#eee;font-size: 9pxcolor:#000; text-align: left;'><div style='padding: 5px;'>PRODUCTS</div></li>";
		 html += create_list(prods, 'products');
		 html += "</ul>";
